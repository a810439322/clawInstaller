...
namespace OpenClawInstaller
{
    public class DeployWorker
    {
        ...
        public async Task RunAsync(IProgress<int> progress, IProgress<string> logger)
        {
            ...
            ps1Builder.AppendLine("        \"3\" { Migrate-Config }");
            ps1Builder.AppendLine("        \"4\" { Open-Terminal }");
            ps1Builder.AppendLine("        \"5\" { exit }");
            ps1Builder.AppendLine("        default { Show-Menu }");
            ps1Builder.AppendLine("    }");
            ps1Builder.AppendLine("}");
            ps1Builder.AppendLine("");

            ps1Builder.AppendLine("function Migrate-Config {");
            ps1Builder.AppendLine("    Clear-Host");
            ...
            ps1Builder.AppendLine("        $overwrite = Read-Host \"是否覆盖? (y/N)\"");
            ps1Builder.AppendLine("        if ($overwrite -ne 'y' -and $overwrite -ne 'Y') {");
            ps1Builder.AppendLine("            Write-Host \"已取消迁移。按任意键返回主菜单...\"");
            ps1Builder.AppendLine("            $null = $host.UI.RawUI.ReadKey(\"NoEcho,IncludeKeyDown\")");
            ps1Builder.AppendLine("            Show-Menu");
            ps1Builder.AppendLine("            return");
            ps1Builder.AppendLine("        }");
            ps1Builder.AppendLine("        Remove-Item -Recurse -Force $targetPath");
            ps1Builder.AppendLine("    }");
            ps1Builder.AppendLine("");
            ps1Builder.AppendLine("    if (-not (Test-Path $sourcePath)) {");
            ps1Builder.AppendLine("        Write-Host \"错误: 未找到源配置目录: $sourcePath\" -ForegroundColor Red");
            ps1Builder.AppendLine("        Write-Host \"请先运行 Onboard 向导或手动指定配置目录。\" -ForegroundColor Yellow");
            ...
            ps1Builder.AppendLine("    }");
            ps1Builder.AppendLine("");
            ps1Builder.AppendLine("    Write-Host \"正在复制配置文件...\"");
            ps1Builder.AppendLine("    Copy-Item -Recurse -Force $sourcePath $targetPath");
            ps1Builder.AppendLine("    Write-Host \"✓ 配置已复制到: $targetPath\" -ForegroundColor Green");
            ps1Builder.AppendLine("    Write-Host \"\"");
            ps1Builder.AppendLine("    Write-Host \"提示: 配置文件中的绝对路径可能需要手动调整。\" -ForegroundColor Yellow");
            ps1Builder.AppendLine("    Write-Host \"按任意键返回主菜单...\"");
            ps1Builder.AppendLine("    $null = $host.UI.RawUI.ReadKey(\"NoEcho,IncludeKeyDown\")");
            ps1Builder.AppendLine("    Show-Menu");
            ps1Builder.AppendLine("}");
            ...
            ps1Builder.AppendLine("    Clear-Host");
            ps1Builder.AppendLine("    Write-Host \"✓ 正在运行 Gateway...\" -ForegroundColor Cyan");
            ps1Builder.AppendLine("    Write-Host \"========================================\"");
            ps1Builder.AppendLine("    $env:OPENCLAW_STATE_DIR = \"$scriptDir\\.openclaw\"");
            ps1Builder.AppendLine("    npx openclaw gateway");
            ps1Builder.AppendLine("    Write-Host \"\"");
